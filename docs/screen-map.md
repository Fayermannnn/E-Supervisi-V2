...
- **Visualisasi data** (server-rendered, murni HTML/CSS — nol JS/dependency, sesuai `docs/dataviz`): `<x-ui.meter>` (rasio terhadap batas — track = langkah lebih terang dari ramp yang sama, isi 4px rounded, label nilai di ujung; `tone`: brand/done/overdue/progress/scheduled/neutral) dan `<x-ui.bar-distribution>` (part-to-whole kategori berwarna token status, celah 2px antar segmen, legenda dot+label+angka — tanpa donut/pie). Dipakai di: Analisis (skor total + per-seksi), Laporan Siklus (skor band), Pelaporan Agregat (siklus per status, RTL per status, % dilaporkan per sekolah), Akuntabilitas 360° (rata-rata per dimensi), Dasbor (sebaran siklus per status), Pelacak RTL (butir selesai/total).
