...
| T-04 | Kepatuhan UU 27/2022 PDP (basis pemrosesan, retensi, enkripsi) | Tinggi | Header keamanan respons (CSP nonce+hash, HSTS, X-Frame-Options, Referrer/Permissions-Policy) via `SecureHeaders` middleware — diuji (ADR-016); enkripsi transit (TLS) + at-rest utk berkas observasi; retensi = arsip bukan hapus; audit menyeluruh; dokumen basis pemrosesan (pelaksanaan tugas dinas) di `deployment.md` | OPEN (sisa: review hukum + basis pemrosesan + enkripsi at-rest deployment) |
